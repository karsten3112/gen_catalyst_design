...
def main():
    ...
    train_loader, val_loader = get_dataloaders_from_atoms_list(
        atoms_list=read("../rate_testing/no_duplicates.traj", index=":"),
        element_pool=element_pool,
        condition_key="rate",
    ...
    for batch in train_loader:
        denoise_matching_term, reconstruction_term, aux_loss, aux_rate_loss = diff_model.calculate_loss_terms(batch=batch, batch_idx=None)
        print(f"denoise_matching_term: {denoise_matching_term}")
        print(f"reconstruction_term: {reconstruction_term}")
        print(f"d3pm_aux_loss: {aux_loss}")
        print(f"aux_rate_loss: {aux_rate_loss}")
        exit()
