...
def main():
    ...
    samples_files = [f"Absorbing_{guidance_scale}_scale.traj", f"Uniform_{guidance_scale}_scale.traj"]
    for sample_file in samples_files:
        atoms_list = read(filename=sample_file, index=":")
        ...
        splitted_file = sample_file.split("_")
        database = Database.establish_connection(
            filename=f"{splitted_file[0]}_{splitted_file[1]}_scale.db",
        ...
        print(f"done evaluating for: {splitted_file[0]} with guidance_scale: {splitted_file[1]}")
