...
def main():
    ...
    condition_embedder = RateEmbedder(embedding_dim=hidden_dim)#ClassLabelEmbedder(num_labels=12, embedding_dim=hidden_dim)
    ...
    diff_model = DiffusionModel(
        ...
        noiser=noiser,
        denoiser=denoiser,
        rate_guidance=None,#rate_guidance,
        drop_prob=0.1,
        use_x0_reparam=True,
        auxillary_weight=0.1,
        auxillary_rate_weight=0.5,
        num_sample_estimate=1,
        lr=1e-3
    )
