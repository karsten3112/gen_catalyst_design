...
from gen_catalyst_design.post_processing import entropy_of_coverage
...
def main():
    filename = "annealing_no_rot_filter.db"
    ...
    step = 0.05
    percentiles = np.arange(0.0, 1.0+step, step)#np.linspace(0.0, 1.0, 20)
    print(percentiles)
    ...
    for i in range(num_bins-1):
        ...
        if len(result_dicts) == 0:
            ...
            for j, site_coverage in enumerate(fractional_coverages):
                site_entropy = entropy_of_coverage(
                    frac_coverage=site_coverage,
