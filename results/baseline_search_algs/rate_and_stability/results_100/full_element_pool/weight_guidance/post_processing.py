...
def main():
    ...
    fig, axs = plt.subplots(1,len(rnd_seeds), figsize=(12,4), sharex=True, sharey=True)
    i = 0
    for rnd_seed, ax in zip(rnd_seeds, axs):
        rate_weight, eform_weight = rate_weights[rnd_seed], eform_weights[rnd_seed]
        ...
        atoms_traj_list = read(
            filename=os.path.join("..", f"rnd_seed_{rnd_seed}_samples.traj"),
            index=":"
        )
        ...
        ax.set_title(r"$w_r="+rf"{rate_weight}$, " r"$w_E="+f"{eform_weight}$")
        if i == 0:
            ax.set_ylabel(r"$E_{form}$ [eV]")
        i+=1

    write_to_yaml(
        "reconstructions.yaml",
        tot_reconstruction_dict
    )
    ...
    plt.savefig("ga_guidance.pdf", bbox_inches="tight", dpi=200)
