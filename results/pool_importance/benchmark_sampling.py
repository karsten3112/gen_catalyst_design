...
def main():
    ...
    train_atoms_list = read("no_duplicates.traj", ":")
    ...
    diff_model = "model_no_att_pool_no_log"
    ...
    for temp in temps:
        print(f"=====================STARTED SAMPLING using T={temp}=====================")
        save_dir = os.path.join(model_pth_header, diff_model, f"temp_{temp}_samples")
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        for i, rate_condition in enumerate(rate_conditions):
            print(f"-------------CONDITION: r={rate_condition:2f},num={i}--------------")
