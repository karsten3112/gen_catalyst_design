...
def main():
    use_log = True
    model_dir = "hyperparam_search"
    tau_sweep = [0,1,3,5,7,10]
    models = [filename for filename in os.listdir(model_dir) if "model" in filename]

    real_dataset = os.path.join("..", "datasets_100", f"genetic_algorithm_2000_no_saas.traj")
    ...
    for model in models:
        samples_dir = os.path.join(model_dir, model, "samples")
        ...
        for g_scale in g_scales:
            ...
            if do_analysis == True:
                ...
                for tau in tau_sweep:
                    ...
                    internal_coverage = compute_internal_coverage(
                        generated_datadicts=generated_positive,
                        tau=tau
                    )
                    ...
                    result_dict = {"tau":tau, "precision":precision, "recall":recall, "novelty":novelty, "internal_coverage":internal_coverage}
        ...
        write_to_yaml(
            filename=os.path.join(model_dir, model, "pr_recall.yaml"),
            data=pr_recall_results
        )

        write_to_yaml(
            filename=os.path.join(model_dir, model, "best_frac.yaml"),
            data=best_fracs
        )
...
def compute_internal_coverage(
    ...
    vectors = [
        get_assignment_vector(datadict=g, label_key=label_key)
        for g in generated_datadicts
    ]
    covered = 0
    for i, vec_i in enumerate(vectors):
        distances = [
            hamming_distance(vec_i, vec_j)
            for j, vec_j in enumerate(vectors)
            if i != j
        ]
        min_dist = min(distances)
        covered += 1 if min_dist <= tau else 0

    return covered/len(vectors)
