...
def main():
    ...
    for opt_method in opt_methods:
        db = Database.establish_connection(
            ...
            pth_header=os.path.join("../../optimization", opt_method, "results_no_saas_fix_100")
        )
        atoms_list_tot = load_atoms_list_from_db(
            database=db
        )
        for num_sample_split in num_sample_splits:
            write(f"{opt_method}_{num_sample_split}_no_saas.traj", images=atoms_list_tot[0:num_sample_split])
