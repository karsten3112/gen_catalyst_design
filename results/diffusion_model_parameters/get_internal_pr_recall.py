...
def main():
    ...
    for i, ax in enumerate(axs):
        ax.set_xlabel("Internal Coverage \%")
        ax.grid()
        for model in diff_model_params:
            ...
            if drop_prob_model == drop_prob:
                ...
                precisions, internal_recalls, g_scales, novelties, best_fracs, recalls = [], [], [], [], [], []
                    ...
                    if pr_recall_dict[g_scale_label] is not None:
                        for pr_recall_info in pr_recall_dict[g_scale_label]:
                            if pr_recall_info["tau"] == tau:
                                ...
                                internal_recalls.append(pr_recall_info["internal_coverage"])
                ...
                for j, g_scale in enumerate(g_scales):
                    ...
                    ax.plot(internal_recalls[j]*100, precisions[j]*100, color=color, **plot_kwargs)
    ax.set_title(rf"$\tau={tau}$ [Tokens]")
    plt.savefig(f"pr_internal_coverage_tau_{tau}.pdf")
