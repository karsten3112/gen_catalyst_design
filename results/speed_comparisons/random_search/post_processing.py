...
def main():
    ...
    parameters = {#"ntasks-per-node":[1,2,3,4,5,6],
                     ...
                     #"nodes":[1,2,3,4,5,6],
                     "partition":["q48"]#"q20","q24", "q28", "q36", "q40", "q48", "qgpu"]
                     }
    ...
    for param in parameters:
        ...
        for value in parameters[param]:
            ...
            log_times = np.array(data_dict["logging_time"])
            eval_times = np.array(data_dict["eval_time_per_batch"])
            ...
            x_steps = np.arange(100, data_dict["batch_size"]*data_dict["batches"]+100, 100)
            eval_time_per_sample = np.mean(eval_times)/data_dict["batch_size"]/data_dict["batches"]
            ax.plot(x_steps, time_spent_arr, label="Random-search", **plot_kwargs)
        ax.legend()
        plt.savefig(f"{param}_rnd.svg")
