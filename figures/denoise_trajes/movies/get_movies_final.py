...
def main():
    ...
    scenarios = ["rate_and_stability"] #"rate_only"

    for scenario in scenarios:
        model_dir = os.path.join("..", scenario)
        models = [file for file in os.listdir(model_dir) if "model" in file]
        rad_scale = 2500 if scenario == "rate_only" else 3000

        for model in models:
            if scenario == "rate_and_stability":
                sample_dirs = [os.path.join("..", scenario, model, miller_index) for miller_index in ["100", "111"]]
            else:
                sample_dirs = [os.path.join("..", scenario, model)]

            for sample_dir in sample_dirs:
                outdir = "\\".join(sample_dir.split("\\")[1:])
                if not os.path.exists(outdir):
                    os.makedirs(outdir)

                traj_files = [file for file in os.listdir(sample_dir) if "traj" in file]
                for traj_file in traj_files:
                    traj_filename = traj_file.split(".")[0]
                    render(
                        input_traj=os.path.join(sample_dir,traj_file),
                        output_mp4=os.path.join(outdir, traj_filename+".mp4"),
                        fps=framerate,
                        dpi=200,
                        stride=frame_skips,
                        elev=25,
